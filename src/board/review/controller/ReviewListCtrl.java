...
public class ReviewListCtrl extends HttpServlet {
	...
	private void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int allPages = 0;
		try {
			allPages = new ReviewService().reviewCnt(search, searchType);
		...
		try {
			list = new ReviewService().getReviewBoard(startRnum, endRnum, search, searchType);
		} catch (SQLException e) {
			e.printStackTrace();
		}
		...
		request.getRequestDispatcher("board/review/rlist.jsp").forward(request, response);
