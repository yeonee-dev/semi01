...
/**
 * Servlet implementation class ReviewWriteCtrl
 */
...
public class ReviewWriteCtrl extends HttpServlet {
	...
	private void execute(HttpServletRequest request, HttpServletResponse response)
		...
		String rtag = mReq.getParameter("rtag");

		vo.setRsubject(mReq.getParameter("rsubject"));
		vo.setRcontent(mReq.getParameter("rcontent"));
		vo.setRtag(Integer.parseInt(rtag));
		vo.setRwriter(rwriter);
		vo.setRfilepath(fileNames);

		int result = 0;
		try {
			result = new ReviewService().reviewWrite(vo);
